"""
    - Using the selected parameter from the Uk weather link we got the .txt URL which provide as input data.
    - Tried to take the country name dynamically as input, So we can pass other country name to fecth data.
    - Parsed the data using numpy and pandas libraries and store the information in the database table
        as methioned below and we can access the stored by GET API.
    - Table columns used to store data in
        column - region - string - Uk or England
        column - information - string - 'Information data'
        column - whether_data - JSON - {'year': 1884, 'jan':7.3, 'feb':6.8 ......}
"""
